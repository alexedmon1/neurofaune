...
from neurofaune.config import load_config, load_yaml, merge_configs, validate_config
...
def _detect_modalities(bids_root: Path) -> List[str]:
    """Auto-detect which modalities are present in BIDS data."""
    modalities = []
    if not bids_root.exists():
        return modalities

    # Check for any files matching modality patterns
    if list(bids_root.glob('sub-*/ses-*/anat/*_T2w.nii*')):
        modalities.append('anat')
    if list(bids_root.glob('sub-*/ses-*/dwi/*_dwi.nii*')):
        modalities.append('dwi')
    if list(bids_root.glob('sub-*/ses-*/func/*_bold.nii*')):
        modalities.append('func')
    # MSME doesn't have a standard BIDS suffix; check for common patterns
    if (list(bids_root.glob('sub-*/ses-*/anat/*MSME*')) or
            list(bids_root.glob('sub-*/ses-*/anat/*msme*'))):
        modalities.append('msme')

    return modalities


def _write_commented_yaml(config: Dict[str, Any], output_path: Path,
                          detected_modalities: Optional[List[str]] = None) -> None:
    """
    Write config as YAML with a header comment explaining the file.
    """
    header = [
        "# Neurofaune Study Configuration",
        f"# Generated: {datetime.now().isoformat()}",
        "#",
        "# This file was generated by neurofaune init_study.py.",
        "# It contains all preprocessing parameters with sensible defaults.",
        "# Edit any values below to customize for your study.",
        "#",
        "# Parameter defaults come from configs/default.yaml (shipped with neurofaune).",
        "# Only parameters you override here will differ from defaults.",
        "#",
    ]

    if detected_modalities:
        header.append(f"# Detected modalities in BIDS data: {', '.join(detected_modalities)}")
    else:
        header.append("# No BIDS data detected — all modality sections included with defaults.")
...
def generate_config(
    ...
    """
    Generate a comprehensive study configuration file.

    Loads package defaults from configs/default.yaml, applies study-specific
    overrides (paths, atlas, cohorts), and writes a well-commented config
    to the study root directory.
    ...
    modalities : list of str, optional
        Modalities to include ('anat', 'dwi', 'func', 'msme').
        If None and BIDS data exists, auto-detect. Otherwise include all.
    ...
    detected_modalities = None
    if modalities is None and bids_root.exists():
        detected_modalities = _detect_modalities(bids_root)
        if detected_modalities:
            print(f"  Auto-detected modalities: {', '.join(detected_modalities)}")
    ...
    study_overrides = {
        ...
        'paths': {
            ...
            'bids': str(bids_root) if bids_root != study_root / 'raw' / 'bids' else '${paths.raw}/bids',
    ...
    # Atlas configuration
    if sigma_atlas_path:
        study_overrides['atlas'] = {
            'name': 'SIGMA',
            'base_path': str(sigma_atlas_path),
            'study_space': {
    ...
    # Merge defaults with study overrides
    config = merge_configs(base_config, study_overrides)

    if output_path is None:
        output_path = study_root / 'config.yaml'

    _write_commented_yaml(config, output_path, detected_modalities)
