...
def _compare_pair(
    ...
    data_a: np.ndarray,
    data_b: np.ndarray,
    observed_a: dict[float, dict],
    observed_b: dict[float, dict],
    densities: list[float],
    n_perm: int,
    seed: int,
) -> list[dict]:
    """Run permutation test for a single pair of groups.

    Standalone function suitable for use with ProcessPoolExecutor.

    Returns a list of result dicts (one per metric per density).
    """
    metric_names = [
        "global_efficiency",
    ...
    for d in densities:
        for m in metric_names:
            ...
            rows.append({
                ...
                "metric": m,
                "density": d,
                "observed_a": observed_a[d][m],
                "observed_b": observed_b[d][m],
                "observed_diff": obs_d,
                "p_value": p_val,
            })
...
def compare_metrics(
    ...
    densities : list[float], optional
        Network densities to test. Default [0.15, 0.20].
    n_perm : int
    ...
    logger.info(
        "Graph metrics comparison: %d pairs, %d permutations, %d densities, %d workers",
        n_pairs, n_perm, len(densities), n_workers,
    )
    ...
    if n_workers > 1 and n_pairs > 1:
        ...
        with ProcessPoolExecutor(max_workers=n_workers) as executor:
            for pair_idx, (label_a, label_b) in enumerate(pairs, 1):
                future = executor.submit(
                    ...
                    observed_a=observed[label_a],
                    observed_b=observed[label_b],
                    densities=densities,
                    n_perm=n_perm,
                    seed=seed + pair_idx,
                )
                futures[future] = (pair_idx, label_a, label_b)

            for future in as_completed(futures):
                pair_idx, label_a, label_b = futures[future]
