...
`scripts/batch_preprocess_for_templates.py` takes no arguments and still hardcodes a
deleted example config — do not use it as-is; point it at your study config first.
See README.md for the verified CLI of every documented script.
...
│   │   └── mrs_preprocess.py    # SVS PRESS spectroscopy
...
- `configs/default.yaml` - package defaults (never edit per-study)
