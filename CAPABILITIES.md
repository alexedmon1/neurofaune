...
_Generated from the code by `neurofaune capabilities` (v0.6.2a0)._ Do not edit by hand — run `make capabilities`.
...
| `run_mrs_preprocessing` | `neurofaune.preprocess.workflows.mrs_preprocess` | Run the full SVS pipeline for one subject/session. | `spectroscopy.basis`, `spectroscopy.fitter`, `spectroscopy.preproc`, `spectroscopy.remove_water` |
