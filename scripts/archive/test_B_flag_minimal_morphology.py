#!/usr/bin/env python3
"""
Test -B flag with minimal morphology.

Strategy: Use -B flag for skull removal, minimal morphological operations.
"""
...
def main():
    ...
    output_dir = Path('/mnt/arborea/bpa-rat/test/skull_strip_B_flag')
    ...
    print("="*80)
    print("TESTING -B FLAG WITH MINIMAL MORPHOLOGY")
    print("="*80)
    print("Strategy: Use -B for clean skull removal, minimal morphology")
    print()
    ...
    # Test configurations with -B flag
    configs = [
        {
            'name': 'B_flag_no_morphology',
            'frac': 0.30,
            'use_B': True,
            'erode': 0,
            'dilate': 0,
            'desc': '-B flag, no morphology'
        },
        {
            'name': 'B_flag_minimal_morphology',
            'frac': 0.30,
            'use_B': True,
            'erode': 1,
            'dilate': 1,
            'desc': '-B flag, erode/dilate 1x'
        },
        {
            'name': 'B_flag_lower_frac',
            'frac': 0.25,
            'use_B': True,
            'erode': 1,
            'dilate': 1,
            'desc': '-B flag, frac=0.25, erode/dilate 1x'
        },
        {
            'name': 'B_flag_higher_frac',
            'frac': 0.35,
            'use_B': True,
            'erode': 1,
            'dilate': 1,
            'desc': '-B flag, frac=0.35, erode/dilate 1x'
        },
        {
            'name': 'B_flag_light_cleanup',
            'frac': 0.30,
            'use_B': True,
            'erode': 2,
            'dilate': 1,
            'desc': '-B flag, asymmetric cleanup'
        }
    ]
    ...
    for config in configs:
        ...
        try:
            brain_file, mask_file, info = skull_strip_bold_preprocessed(
                ...
                clean_mask=config['erode'] > 0,  # Only clean if erode > 0
    ...
    # Summary
    print(f"\n{'='*80}")
    print("SUMMARY - -B Flag with Minimal Morphology")
    print('='*80)
    ...
    for result in results:
        ...
        print(f"\n{config['name']}:")
        print(f"  {config['desc']}")
        print(f"  Parameters: frac={config['frac']}, -B=True, erode={config['erode']}x, dilate={config['dilate']}x")
