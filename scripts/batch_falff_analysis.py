#!/usr/bin/env python3
"""
Batch fALFF analysis: compute fALFF/ALFF maps and warp to SIGMA space.

Discovers all sessions with preprocessed BOLD data and computes:
- fALFF (fractional ALFF) from unfiltered regressed data
- ALFF maps
- Z-scored versions of both
- SIGMA-space warping of fALFF/ALFF maps (optional)
...
def process_session(session_info: dict, config: dict,
    ...
    try:
        ...
        if not skip_sigma and has_sigma:
            ...
            # Collect native-space fALFF/ALFF maps to warp
            input_files = {}
            desc_map = {
            ...
            if input_files:
                ...
                print("  No native-space fALFF/ALFF maps found for SIGMA warping")
        ...
        metadata_file = deriv_dir / f"{subject}_{session}_desc-falff_analysis.json"
        metadata = {
            "subject": subject,
            "session": session,
...
def main():
    parser = argparse.ArgumentParser(
        description="Batch fALFF analysis with optional SIGMA-space warping"
    )
    parser.add_argument(
        "--study-root",
        type=Path,
    ...
    parser.add_argument(
        "--skip-sigma",
        action="store_true",
        help="Skip SIGMA-space warping (only compute fALFF/ALFF in native space)",
    )
